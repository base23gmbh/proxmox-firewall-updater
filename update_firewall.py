...
class FirewallEntry:
    ...
    def domain(self) -> str | None:
        """Extract domain from comment if it contains #resolve: directive."""
        try:
...
def update_firewall_objects(deps: Dependencies, obj_type: FirewallObjectType):
    ...
    entries = [entry for entry in deps.list_entries(obj_type) if entry.domain() is not None]
    
    if deps.verbose:
        log(f'Found {len(entries)} {type_name.lower()} entries to check. dry-run={deps.dry_run}')
        for entry in entries:
            log(f'  {entry.name} {entry.domain()} cidr={entry.cidr} {entry.comment}')
    
    for entry in entries:
        # Get all IP addresses from DNS
        dns_ips = deps.dns_resolve(entry.domain())
        
        if dns_ips:
            if obj_type == FirewallObjectType.IPSET:
                # For IPSets, handle multiple IP addresses
                # Get current entries in the IPSet
                current_ips = deps.get_object_entries(obj_type, entry.name)
                
                if deps.verbose:
                    log(f'{type_name} {entry.name} has {len(current_ips)} entries, DNS returned {len(dns_ips)} addresses')
            ...
            else:  # Alias objects only support a single IP
                ...
                if ip != entry.cidr:
                    ...
                    if deps.verbose:
                        log(f'{type_name} {entry.name} is already up to date with {ip}')
        else:
            if deps.verbose:
                log(f'Cannot resolve domain `{entry.domain()}` for {type_name} `{entry.name}`')
...
class ProdDependencies(Dependencies):
    ...
    def set_entry(self, entry: FirewallEntry):
        ...
        if entry.obj_type == FirewallObjectType.IPSET:
            # For IPSets
            # First, make sure the IPSet exists with the right comment
            create_cmd = f'pvesh create cluster/firewall/ipset --name {entry.name}'.split(' ')
            if entry.comment:
                create_cmd += ['--comment', entry.comment]
            self._run(create_cmd, skip=self.dry_run)
